...
class ResumeManager {
  ...
  // Resume specific helper with improved error handling and fallback
  public static async uploadResume(file: File): Promise<UploadResult> {
    ...
    // Try unsigned upload first (if preset is configured)
    if (this.validateEnvironmentUnsigned()) {
      console.log('Attempting unsigned upload with preset:', this.UNSIGNED_PRESET);
      ...
      if (unsignedResult.success && unsignedResult.url) {
        // Return immediately on success; avoid extra HEAD checks that may 401
        return unsignedResult;
      } else {
        console.warn('Unsigned upload failed:', unsignedResult.error);
      }
    } else {
      console.warn('Unsigned upload not configured, falling back to signed upload');
    }

    // Fallback to signed upload if unsigned fails or is not configured
    if (this.validateEnvironment()) {
      console.log('Attempting signed upload as fallback');
      ...
      return { 
        ...
        For unsigned uploads, you need CLOUD_NAME and UPLOAD_PRESET.
        For signed uploads, you need CLOUD_NAME, API_KEY, and API_SECRET.` 
      };
    }
  }
